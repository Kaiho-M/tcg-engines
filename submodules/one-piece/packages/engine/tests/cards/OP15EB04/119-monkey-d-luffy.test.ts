...
import { op15eb04MonkeyDLuffy119 } from "../../../../cards/src/cards/OP15EB04/characters/119-monkey-d-luffy.ts";
...
describe("OP15-119 Monkey.D.Luffy", () => {
  test("when the opponent blocks, reveals the top Life card and gains +1000 per cost on it for the turn", () => {
    const engine = OnePieceTestEngine.create(
      {
        character: [{ card: op15eb04MonkeyDLuffy119, playedOnTurn: 0 }],
    ...
    const luffyId = engine.findCardInZone("south", "character", op15eb04MonkeyDLuffy119);
    ...
    expect(getCardPower(engine.getState(), luffyId)).toBe(op15eb04MonkeyDLuffy119.power! + 4000);
