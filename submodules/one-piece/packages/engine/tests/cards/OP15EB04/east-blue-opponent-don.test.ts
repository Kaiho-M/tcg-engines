...
import { op15eb04Alvida003 } from "../../../../cards/src/cards/OP15EB04/characters/003-alvida.ts";
import { op15eb04Mohji018 } from "../../../../cards/src/cards/OP15EB04/characters/018-mohji.ts";
import { op15eb04MeowbanBrothers028 } from "../../../../cards/src/cards/OP15EB04/characters/028-meowban-brothers.ts";
import { op15eb04Krieg001 } from "../../../../cards/src/cards/OP15EB04/leaders/001-krieg.ts";
...
describe("OP15 East Blue: giving the opponent's DON!! to their own cards", () => {
  test("OP15-028 moves one of the opponent's active DON!! onto their Character", () => {
    const engine = OnePieceTestEngine.create(
      { leaderCardId: op15eb04Krieg001, hand: [op15eb04MeowbanBrothers028], activeDon: 1 },
      { character: [eb01Doma005], activeDon: 3 },
      { firstPlayer: "north", activeSeat: "south" },
    );
    const domaId = engine.findCardInZone("north", "character", eb01Doma005);

    engine.playCard(op15eb04MeowbanBrothers028, "south");
  ...
  test("OP15-003 pays the opponent's rested DON!! onto a chosen opposing Character", () => {
    const engine = OnePieceTestEngine.create(
      { character: [{ card: op15eb04Alvida003, playedOnTurn: 0 }] },
      { character: [eb01Doma005, eb01MountainGod018], restedDon: 1, activeDon: 2 },
      { firstPlayer: "north", activeSeat: "south" },
    );
    const alvidaId = engine.findCardInZone("south", "character", op15eb04Alvida003);
    const domaId = engine.findCardInZone("north", "character", eb01Doma005);
  ...
  test("OP15-018 can only K.O. an opposing Character that has DON!! given", () => {
    const engine = OnePieceTestEngine.create(
      { character: [{ card: op15eb04Mohji018, playedOnTurn: 0 }] },
      { character: [{ card: eb01Doma005, attachedDon: 1 }, eb01Doma005] },
      { firstPlayer: "north", activeSeat: "south" },
    );
    const mohjiId = engine.findCardInZone("south", "character", op15eb04Mohji018);
