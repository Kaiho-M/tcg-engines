...
import { op16MonkeyDLuffy015 } from "../../../../cards/src/cards/characters/op16-015-monkey-d-luffy.ts";
import { op16PortgasDAce001 } from "../../../../cards/src/cards/leaders/op16-001-portgas-d-ace.ts";
import { op17EdwardNewgate005 } from "../../../../cards/src/cards/characters/op17-005-edward-newgate.ts";
...
describe("OP16-015 Monkey.D.Luffy", () => {
  ...
  test("a competing 'base power becomes 8000' keeps the higher value (4-9-2-1) without adding", () => {
    const engine = OnePieceTestEngine.create(
      {
        leaderCardId: op16PortgasDAce001,
        character: [op16MonkeyDLuffy015],
    ...
    acceptLuffyTrigger(engine);
    // 4-9-2-1: several effects setting a base power apply the highest value.
    expect(leaderPower(engine)).toBe(8000);
    expect(characterPower(engine, luffyId)).toBe(7000);
  });
});
